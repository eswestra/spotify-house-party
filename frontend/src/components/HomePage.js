    ...
    <Grid container spacing={3} align='center' className={classes.paper}>
      ...
      <Grid item xs={12}>
        <ButtonGroup disableElevation variant='contained' color='primary'>
          <Button to='/rooms/join' component={Link}>Join Room</Button>
          <Button to='/rooms/create' component={Link} color='secondary'>Create Room</Button>
        </ButtonGroup>
      </Grid>
    </Grid>
  )
  ...
  useEffect(() => {
	...
	return(
        ...
        <Route exact path='/'  render={() => {
          return userRoomCode ? <Redirect to={`/rooms/${userRoomCode}`} /> : <HomePage />
        }}/> 
